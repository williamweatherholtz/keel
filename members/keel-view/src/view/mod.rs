...
pub fn inversion_pairs(ready: &[(String, Option<String>)], recorded: &HashSet<String>) -> Vec<(String, String, String)> {
    ...
    for (i, (high, sev)) in ready.iter().enumerate() {
        ...
        for (lower, lsev) in ready.iter().take(i) {
            if lsev.as_deref().is_none_or(|l| !at_least_high(l)) && !recorded.contains(lower) {
...
pub fn arch_model(root: &Path) -> Result<ArchModel, ViewError> {
    let model = Model::build(root)?;
    // A `#Supersede` edge retires its target whole (invariant 4): a CodeElement whose file moved is
    // followed by a successor element and such an edge (D0108), and the retired one must not keep
    // reporting its old path as MISSING drift (sprint 718, issue tracked with this test).
    let retired: HashSet<&str> = model.edges.iter().filter(|e| e.kind == "supersede").map(|e| e.to.as_str()).collect();
    let mut elements: Vec<CodeElementRow> = model
        .items
        .iter()
        .filter(|(n, i)| i.type_name == "CodeElement" && !retired.contains(n.as_str()))
        .map(|(n, i)| {
            let g = |k: &str| i.attrs.get(k).cloned().unwrap_or_default();
            CodeElementRow {
                name: n.clone(),
...
mod tests {
    ...
    fn inversion_pairs_flags_lower_severity_work_ranked_above_high() {
        ...
        let out = inversion_pairs(&ready, &HashSet::new());
        assert_eq!(
            out,
            vec![
        ...
        // D0429: a recorded rank exempts the outranking item and no other; removing the record
        // restores its warning.
    ...
    fn inversion_pairs_clean_when_severity_matches_order() {
        ...
        assert!(inversion_pairs(&ready, &HashSet::new()).is_empty(), "severity-ordered frontier must be clean");
        // Two >= High items may outrank each other freely — relative order among them is judgment.
        assert!(inversion_pairs(&[("a".to_string(), sev("High")), ("b".to_string(), sev("Critical"))], &HashSet::new()).is_empty());
    }
