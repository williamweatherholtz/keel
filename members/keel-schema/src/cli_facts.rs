...
pub const CLI_FACTS: [CliFact; 86] = [
    CliFact { name: "accept", family: "governance", effect: "writes", stability: "stable", invocation: "<decision> (--words TEXT | --note-from FILE | --note TEXT) --by <person> --date YYYY-MM-DD", synopsis: "record a human's acceptance of a proposed Decision; refused for an AI actor" },
    CliFact { name: "reject", family: "governance", effect: "writes", stability: "stable", invocation: "<decision> (--words TEXT | --note-from FILE | --note TEXT) --by <person> --date YYYY-MM-DD", synopsis: "record a human's rejection of a proposed Decision; the same channel rules as accept" },
    CliFact { name: "judge-set", family: "governance", effect: "writes", stability: "stable", invocation: "<.tracking file> (--words TEXT | --note-from FILE | --note TEXT) --by <person> --date YYYY-MM-DD [--verdict pass|fail] [--fail <test>,..] [--all]", synopsis: "record a human's verdict on the sampled proposed results of one file - one result and one quote receipt per item (D0443); accept's channel rules under confirmationRecord" },
    CliFact { name: "override", family: "governance", effect: "writes", stability: "stable", invocation: "<path> (--reason-from FILE | --reason TEXT)", synopsis: "arm a single-use, path-bound write unlock; consuming it records an obligation" },
    ...
    CliFact { name: "record", family: "authoring", effect: "writes", stability: "stable", invocation: "decision|issue|statement ... | story --from-statement stNNN --title T --as-a ROLE (--i-want-from FILE | --i-want TEXT) --implication K [--so-that-from FILE | --so-that TEXT] [--triage-note-from FILE | --triage-note TEXT] | task --file F --def D --task T --method M --dod-from FILE | sprint <N> <slug> --charter <dNNNN> [--points P] [--fill FILE] | result --file F --task T --sha S [--verdict pass|fail] --judged-by A --judged-at D [--evidence-from FILE | --evidence TEXT] | gate-result --file F --gate G --sha S [--verdict pass|fail] --judged-by A --judged-at D [--evidence-from FILE | --evidence TEXT] [--notes-from FILE | --notes TEXT] | review --batch FILE [--sha S] [--judged-by A] [--judged-at D] | measurement --indicator I --value V [--at DATE] | indicator-snapshot [ROOT] | reverify [--all-drift | --task N | --demos] [--by A] | mint [N]", synopsis: "record one atomic fact, each sub-verb named for what it writes: a Decision, an Issue with its resolver, a human's words verbatim, the story translating them, a task with its Definition of Done, a sprint's ceremony scaffold (--fill never writes a result, D0301), a verdict on a task's DoD or a ceremony gate, a review batch as linked critiques, an indicator measurement or a dated snapshot, fresh gate results at HEAD where the gate is green (--demos replays replayable receipts, D0444); mint prints engine-minted v4 UUIDs and writes nothing" },
    CliFact { name: "gate", family: "gating", effect: "reads", stability: "stable", invocation: "validate [ROOT] | check FILE... | --spec-version | check-engine [ROOT] | guard [NAME] [ROOT] | rules [--enforce] [ROOT] | assured [ROOT] | adoption-check [ROOT] [--unit N] [--keep] [--vintage VERSION] | --fast [ROOT] | --workspace [ROOT]", synopsis: "the gating family under one router (D0452): validate is the semantic authority over every .tracking file; check parses .sysml files or reports the baked grammar version; check-engine resolves .engine instance references kernel-free; guard runs every enforced honest-state guard or one by name; rules lists the declared rules and whether each holds; assured is the composite READY / NOT-READY verdict; adoption-check gates a foreign tree unit by unit; --fast is the per-edit tier and --workspace gates every project the commit touches (bare `keel gate` prints this usage)" },
    CliFact { name: "audit", family: "gating", effect: "reads", stability: "stable", invocation: "history [--since REF] [--max N] | adherence [--since REF] | ci-runs [ROOT] [--repo OWNER/NAME] | [ROOT]", synopsis: "retrospective adherence: charter, ceremony, estimation, sitting review; history re-derives the gate verdict per commit over a range, adherence re-derives guard-set and severity monotonicity per commit so a control cannot be disarmed unsigned, ci-runs is the external-fact gate where every ci-run receipt names a real green run on the judged SHA (D0323)" },
    CliFact { name: "hook", family: "gating", effect: "tooling", stability: "internal", invocation: "post-edit|stop|pre-bash|pre-write|subagent-start|subagent-stop|user-prompt|config-change", synopsis: "the in-loop gates the harness calls; not for a human to invoke" },
    ...
    CliFact { name: "github", family: "channel", effect: "both", stability: "stable", invocation: "pull --repo O/N --by ACTOR --at DATE [--limit N] [--trust T] | ingest --repo O/N --issue N --by ACTOR --at DATE [--from FILE] | decider [<login>] | gesture (env: COMMENT_BODY ...) | decision-id <id>", synopsis: "the channel family under one router (D0453): pull ingests every open issue no Statement cites as a verbatim Statement, autonomy following repository visibility (D0264); ingest records one issue, idempotent on its URL; decider says who may decide on the channel, an unmapped login refused, never defaulted; gesture parses a channel comment into a JSON verdict; decision-id splits a channel decision id into project and name - decider, gesture and decision-id are deprecated with the channel workflow they served (D0289)" },
    ...
    CliFact { name: "verify", family: "gating", effect: "both", stability: "stable", invocation: "[ROOT] [--probe POSITIVE,NEGATIVE | --probe-from FILE] [--no-receipt] | --wait [ROOT]", synopsis: "the pre-commit checks as one ladder in cost order - gate validate, gate guard (from its receipt, D0371), cargo clippy -D warnings, the D0388 probe pair named by --probe POSITIVE,NEGATIVE or read from --probe-from FILE (two lines, the known-positive command then the known-negative, written by the actor who chose them; any other shape or both flags is refused before a rung runs, D0500), then suite --touched (keyed per D0474) - stopping at the first red; one receipt names the rung it stopped at, a rung after the red is not-run, an unnamed pair is not-named, and on green the touched receipt is the one keel land honours (D0476); --wait launches nothing: it blocks on that receipt while its writer's pid is alive, then prints the finished table and exits as the ladder did, and a running stub whose writer is gone is KILLED during <rung>, exit 2, never a verdict (issue575)" },
