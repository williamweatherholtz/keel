...
pub fn cmd_land(repo: &Path, max_attempts: u32) -> i32 {
    ...
    // THE PUSH GATE IS WITHDRAWN (D0356, on the owner's instruction 2026-09-06). It required a green
    // full-suite receipt covering the deliverable before any push. Measured honestly, the run costs
    // ~11 wall minutes every time the code moves, against roughly one bad push in twenty-five that it
    // could actually catch - two of the three most recent failures were platform faults this machine
