...
"""Build the standing decision brief for the 2026-09-18 (forty-sixth) queue change: the sixteen held records of the forty-fifth
page are joined by a seventeenth held process change whose guard has LANDED under an accepted charter: a file.rs:N citation on
the living doc surface (processes, skills, docs, contracts, workflows, rules, CLAUDE.md) resolves to a source file under a
workspace member and holds the identifier cited beside it - guard source-reference, the seventy-sixth, red at the doc line that
names a moved module's stale line. Two docs cited migrate.rs:664-666 after D0479 moved check_preconditions to line 804 and
nothing read the citation (issue591). The guard's first touched run reddened every scaffold-and-gate test - a new project's
engine docs cite keel's source, which it does not hold - so a root with no Cargo.toml scans nothing, and that sentence is in the
clause. D0514 is the guard's own clause (D0303), standing on D0471 and D0209, held because it adds a hard guard to the enforced
set. Seventeen tabs, seventeen asks (D0404); every count is a facts.py fact, never typed; each metric says in plain words what it
is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_18_source_reference.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502", "d0503", "d0504", "d0505", "d0506", "d0508", "d0509", "d0510", "d0513", "d0514"]
...
<div class="ask"><p class="verdict"><strong>Accept all seventeen.</strong></p></div>
...
TITLE = "Accept seventeen: docs cite code"
SUB = "Seventeen held, unforked"
