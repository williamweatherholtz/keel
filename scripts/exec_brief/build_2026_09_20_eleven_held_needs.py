...
"""Build the standing decision brief for the 2026-09-20 (fifty-seventh) queue change. The ten held Decisions of the
fifty-sixth page still wait and a fifth held process change joins them: D0538, the verifier's receipt is read against
the ladder that ended (check_receipt.py: keel verify --wait, then verify-receipt.toml and touched-receipt.toml; sprint 755).
The seven Need sets are the fifty-fifth page's, re-read. Eighteen tabs, eighteen asks (D0404): eleven Decisions and seven
Need sets. Every count is a facts.py fact, never typed; the new tab's numbers are the check's own probe table run over its
fixtures (facts.py section 60).

Usage: python scripts/exec_brief/build_2026_09_20_eleven_held_needs.py <facts.json> <previous.html> <out.html> [--carry VERDICTS.json]
"""
import json
import re
import sys
...
# ---- the queue: eleven held Decisions and the Needs nobody asked about -----------------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0525", "d0527", "d0528", "d0529", "d0530", "d0531", "d0532", "d0533", "d0536", "d0537", "d0538"]
...
DEC = {"d0525": F["surface"], **F["forks"], **{d: G["held"][d] for d in CHANGES[:3]}, "d0537": T["held"], "d0538": E["held"]}
for _d in FORKS:
    _x = DEC[_d]
...
if T["queue"]["decisionRows"] != QUEUE:
    sys.exit(f"refusing: the authority queue's decision rows must be the eleven: {T['queue']}")
...
if E["queue"]["decisionRows"] != QUEUE:
    sys.exit(f"refusing: the authority queue's decision rows must be the eleven: {E['queue']}")
...
if not (Q["exit0"] and Q["byKind"].get("decisionAcceptance") == pending and Q["decisionRows"] == QUEUE):
    sys.exit(f"refusing: the authority queue's decision rows must be the eleven: {Q}")
...
N_PROPOSED_GATES = S752["gateProposed"] + S754["gateProposed"] + S755["gateProposed"]
...
<div class="ask"><p class="verdict"><strong>Take the recommended course on all eleven</strong>, accept each Need set; choose on every tab, then <em>Record</em>.</p></div>
...
TITLE = "Accept all eleven as recommended and the seven sets of Needs as written"
SUB = "Six forks, five process changes, and the Needs nobody asked you about"
