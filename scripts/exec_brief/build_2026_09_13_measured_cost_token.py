...
"""Build the standing decision brief for the 2026-09-13 (twenty-second) queue change: the twenty-first publish's five asks
stand (the retro name D0461, the keystone's second source D0465, the sample fork D0466, the Issue-first retro D0467, the
host-cost thresholds D0468) and one joined them. D0469 is a process-change ratification held under D0337: a PROPOSED
#ProspectiveChange Decision whose decision text names land, push, refuse or gate as a whole word carries a MEASURED: token
in its rationale or the decision-scaffolding guard WARNS naming it - the clause is inert until D0469 itself is human-accepted.
Origin: issue444 - D0421 priced the land gate from its fixture ('about two seconds') and the first live set cost 528 s. The
live consequence is COMPUTED here from the marked Decisions (facts.py section 23 mirrors guards.rs unmeasured_path_decisions)
and the builder refuses unless the would-warn set is exactly the one the Decision states (d0467, for the word gate).
One tab per ask (D0404); the five carried tabs are the twenty-first builder's, every count a facts.py fact (sections 21-23),
never typed.

Usage: python scripts/exec_brief/build_2026_09_13_measured_cost_token.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: five ratifications and one fork -------------------------------------------------------
QUEUE = ["d0461", "d0465", "d0466", "d0467", "d0468", "d0469"]
...
if [p["slug"] for p in members] != QUEUE:
    sys.exit(f"refusing: this page is written for the retro name, the keystone, the sample fork, the Issue-first retro, the host-cost thresholds and the measured-cost token; the queue is {[p['slug'] for p in members]}")
if AQ["decisions"] != QUEUE:
    sys.exit(f"refusing: the authority-queue lens reads {AQ['decisions']}; the file-derived set is {QUEUE}")
if [p["fork"] for p in members] != [False, False, True, False, False, False]:
...
<p><strong>True in the model:</strong> {UNACC} unaccepted marked Decisions; {len(NAMING)} name a word; {len(WOULD_WARN)} would be named, for <q>not a gate</q>. This item names all four and carries its token. Measured here: the guard {B_MED} s before the clause ({'/'.join(str(x) for x in B_RUNS)}), {W_MED} s with it ({'/'.join(str(x) for x in W_RUNS)}); the armed path is not measurable before the word arming it. <strong>Mine:</strong> one passing mention named is cheaper than a semantic test fitted to one tree. <strong>What decides it:</strong> whether one name on <q>not a gate</q> is noise you will read past. <strong>Wrong if</strong> a proposed path Decision carries a false MEASURED: figure - the token reads presence, not truth.</p>
...
<div class="ask"><p class="verdict"><strong>Accept the five controls</strong> and <strong>read the recall rule at {N100} cases</strong> so {MID} stands. <strong>Wrong if</strong> a one-position median move at {N50} cases is signal to you.</p></div>
...
TITLE = f"Accept five controls, and read the recall rule at {N100} cases so its setting stands"
SUB = "Five ratifications and one fork"
...
print(f"wrote {out_path}: {len(page)} bytes; frame {frame_words} words; panels " +
      ...
      f"; summed {words(page)}; would-warn {PW['wouldWarn']} of {UNACC} unaccepted marked ({PROPOSED_MARKED} proposed, {REJECTED} rejected); "
