...
"""Build the standing decision brief for the 2026-09-16 (thirty-fourth) queue change: the two held process changes
(stems row, CI-triple lint) are joined by a third - the script probes CI runs run before the commit, from ONE runner
both ci.yml and the pre-commit hook call, so the third member of the local-gate-differs-from-CI class cannot recur by
drift. Three tabs, three asks (D0404); every count is a facts.py fact, never typed; each metric says in plain words
what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_16_script_probes.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: three held process changes -----------------------------------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0494", "d0495", "d0496"]
...
# -- ask 3 (first tab): one script-probe runner for both surfaces --
...
figP3 = downstream(
    ...
    [
        ...
        ("Issues in the class", f"the next resolver sits {POS_WAIT_NOW} of {N_BACKLOG}", f"{N_CLASS}", "accent"),
    ],
)
