...
"""Build the standing decision brief for the 2026-09-16 (thirty-ninth) queue change: the eight held records (six process
changes - stems row, CI-triple lint, probe runner, wait command, guard tests, pair file - and the two safety changes from
issue578) are joined by a ninth held process change from sprint 732, the third D0479 extraction: the enforced guard-name
list GUARD_NAMES is declared in members/keel-schema/src/guard_names.rs, re-exported by guards.rs under its old path, and
that file joins GUARD_SOURCE_FILES so deleting a name is the same locked edit as deleting its arm (D0503, the sprint-718
D0486 pattern met again). Nine tabs, nine asks (D0404); every count is a facts.py fact, never typed; each metric says in
plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_16_guard_names_lock.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: seven held process changes and two held safety changes -------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502", "d0503"]
...
PFR = PFL["probeRung"]
# the receipt on disk is the LATEST ladder's - sprint 732's, whose pair is the guard-name sensor probe and the sprint's own
# negative (the third sprint to carry its pair in a file); the page says which file and whose sides, it does not require
...
if not ("module_home.py" in PFR["sides"][0] and "guard_names" in PFR["sides"][0] and "neg732" in PFR["sides"][1]):
    sys.exit(f"refusing: the page says the latest ladder's pair is the guard-name sensor probe and sprint 732's negative: {PFR['sides']}")
...
<p><strong>True:</strong> {N_REFUSALS} wrong shapes refused live, exit 2, receipt untouched; the latest ladder - the third sprint to use it - ran its pair from <q>{PAIR_FILE}</q>, {N_RUNGS_PF} rungs green. <strong>Mine:</strong> a path is the one carrier a verifier cannot mis-copy. <strong>What decides it:</strong> may a subagent retype an argument another actor chose. <strong>Wrong if</strong> a verifier ever needs a pair the primary did not write.</p>
...
<div class="ask"><p class="verdict"><strong>Accept all nine</strong>; two are safety changes.</p></div>
...
TITLE = "Accept the guard-name list's new home and eight more process changes"
SUB = "Nine held, two safety changes, no fork"
