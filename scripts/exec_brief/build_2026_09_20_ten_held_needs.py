...
"""Build the standing decision brief for the 2026-09-20 (fifty-sixth) queue change. The nine held Decisions of the
fifty-fifth page still wait and a fourth held process change joins them: D0537, the recorder's report is read against
the TestResult parts the tree gained since HEAD (check_report.py refusal 9, sprint 754). The seven Need sets are the
fifty-fifth page's, re-read. Seventeen tabs, seventeen asks (D0404): ten Decisions and seven Need sets. Every count is
a facts.py fact, never typed; the new tab's numbers are the check's own probe run over this tree (facts.py section 59).

Usage: python scripts/exec_brief/build_2026_09_20_ten_held_needs.py <facts.json> <previous.html> <out.html> [--carry VERDICTS.json]
"""
import json
import re
import sys
...
# ---- the queue: ten held Decisions and the Needs nobody asked about -----------------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0525", "d0527", "d0528", "d0529", "d0530", "d0531", "d0532", "d0533", "d0536", "d0537"]
...
DEC = {"d0525": F["surface"], **F["forks"], **{d: G["held"][d] for d in CHANGES[:3]}, "d0537": T["held"]}
for _d in FORKS:
    _x = DEC[_d]
...
if T["queue"]["decisionRows"] != QUEUE:
    sys.exit(f"refusing: the authority queue's decision rows must be the ten: {T['queue']}")
...
if not (Q["exit0"] and Q["byKind"].get("decisionAcceptance") == pending and Q["decisionRows"] == QUEUE):
    sys.exit(f"refusing: the authority queue's decision rows must be the ten: {Q}")
...
N_PROPOSED_GATES = S752["gateProposed"] + S754["gateProposed"]
...
<div class="ask"><p class="verdict"><strong>Take the recommended course on all ten</strong>, accept each set of Needs; choose on every tab, then <em>Record</em>.</p></div>
...
TITLE = "Accept all ten as recommended and the seven sets of Needs as written"
SUB = "Six forks, four process changes, and the Needs nobody asked you about"
