...
"""Build the standing decision brief for the 2026-09-18 (forty-eighth) queue change: the eighteen held records of the forty-seventh
page are joined by a nineteenth held process change, applied under its marker as D0492's was: the recorder's report checker gains
an eighth refusal - a REFUSED: line whose reason cites the verifier receipt's noted-writes line (OWED WRITES, or its new label
VERIFIER-NOTED WRITES) is refused, because that line lists writes the VERIFIER noticed for the recorder and never what the
recorder owes; the dispatch's --owed count is the only owed count. Sprint 735's second recorder read `OWED WRITES: NONE` as its
own count, refused its one owed record citing it, and the checker passed the report (issue590). Two fixtures join PAIRS and the
receipt line is relabelled on every surface that names it. D0516 stands on the owed count, the once-per-target rule and the
verifier procedure; held because it changes the delegated-ceremony process. Nineteen tabs, nineteen asks (D0404); every count is
a facts.py fact, never typed; each metric says in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_18_noted_writes.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502", "d0503", "d0504", "d0505", "d0506", "d0508", "d0509", "d0510", "d0513", "d0514", "d0515", "d0516"]
...
# -- ask 19 (first tab): a recorder's REFUSED line cannot cite the verifier's noted-writes line - refusal 8, applied under the held marker --
...
{clause(NW)}
<p><strong>What happened:</strong> the dispatch owed one record; the receipt's line read <q>OWED WRITES: NONE</q>. The recorder took it as its own count, refused citing the line, and the checker passed a report that wrote nothing. The line lists what the verifier noticed; it never counts what the recorder owes.</p>
...
<div class="ask"><p class="verdict"><strong>Accept all nineteen.</strong></p></div>
...
TITLE = "Accept nineteen: refuse misreads"
SUB = "Nineteen held"
