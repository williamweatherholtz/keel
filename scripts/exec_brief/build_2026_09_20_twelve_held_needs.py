...
"""Build the standing decision brief for the 2026-09-20 (fifty-eighth) queue change. The eleven held Decisions of the
fifty-seventh page still wait and a sixth held process change joins them: D0539, every row of guards.md names the family
that dispatches the guard and doc-guard-count holds the cell to the code (sprint 756; issue584, issue667). The seven Need
sets are the fifty-fifth page's, re-read. Nineteen tabs, nineteen asks (D0404): twelve Decisions and seven Need sets. Every
count is a facts.py fact, never typed; the new tab's numbers are the catalogue counted and the guard run live on this tree
(facts.py section 61).

Usage: python scripts/exec_brief/build_2026_09_20_twelve_held_needs.py <facts.json> <previous.html> <out.html> [--carry VERDICTS.json]
"""
import json
import re
import sys
...
# ---- the queue: twelve held Decisions and the Needs nobody asked about -----------------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0525", "d0527", "d0528", "d0529", "d0530", "d0531", "d0532", "d0533", "d0536", "d0537", "d0538", "d0539"]
...
T = v("tenHeldTheReportReadAgainstTheTree")
E = v("elevenHeldTheReceiptReadAgainstTheLadder")
H = v("twelveHeldTheCatalogueNamesTheFamily")
DEC = {"d0525": F["surface"], **F["forks"], **{d: G["held"][d] for d in CHANGES[:3]}, "d0537": T["held"], "d0538": E["held"], "d0539": H["held"]}
for _d in FORKS:
    _x = DEC[_d]
...
# the tenth: the check must be live on this tree - its own probe table run here, every pair holding - and the sprint that
# shipped it must carry its results; the issue it resolves must have left the open list
C = T["check"]
if not (C["docstringSaysNine"] and C["gainedFunction"] and C["pureTakesGained"] and C["refusalNamesUnclaimed"] and C["passLineCountsGained"]
        and C["probeExit0"] and C["probeEveryPairHolds"] and C["probeRows"] == C["fixtures"] == 12
        and C["probePositive741"] and "REFUSED naming" in C["probePositive741"] and C["probeNegative741"] and "-> PASS" in C["probeNegative741"]):
    sys.exit(f"refusing: the report-against-tree check must be live and its probe table must hold on this tree: {C}")
if not (T["skill"]["refusalNineListed"] and T["skill"]["ruleSeven"] and T["skill"]["probePairNamed"]):
    sys.exit(f"refusing: the ceremony skill must carry refusal nine, rule seven and the probe pair: {T['skill']}")
S754 = T["sprint754"]
if not (S754["exists"] and S754["chartered"] and S754["gateResults"] == 6 and S754["storyDodPass"] and S754["dodOnBacklog"] and S754["retroNamesIssue663"]):
    sys.exit(f"refusing: the check's delivery sprint must be chartered by the record with six gate results and both DoD results: {S754}")
if not (T["issue602"]["declared"] and T["issue602"]["openIssuesExit0"] and T["issue602"]["open"] is False):
    sys.exit(f"refusing: the misfiled-write issue must be declared and resolved: {T['issue602']}")
for _n, _x in T["retroIssues"].items():
    if not (_x["declared"] and _x["resolver"]):
        sys.exit(f"refusing: retro issue {_n} must be declared with a resolver: {_x}")
if T["queue"]["decisionRows"] != QUEUE:
    sys.exit(f"refusing: the authority queue's decision rows must be the twelve: {T['queue']}")
N_PROBE_ROWS = C["probeRows"]
# the eleventh: the receipt check must be live - pure refusals(), a --wait step, its probe table holding over the sprint 743
# fixtures - named by both skills, the process trigger and the agent; the sprint that shipped it carries its results; issue603 closed
CR = E["check"]
if not (CR["docstringNamesIssue"] and CR["pureSignature"] and CR["runsWait"] and CR["passLine"] and CR["refusedLine"]
        and CR["probeExit0"] and CR["probeEveryPairHolds"] and CR["probeRows"] == 4 and CR["fixtures"] == 7
        and CR["probePositive743"] and "REFUSED naming `stopped at touched fail (101)`" in CR["probePositive743"]
        and CR["probeNegative743"] and "-> PASS" in CR["probeNegative743"]
        and CR["probeOtherRun"] and "the receipt describes the run at=1789728746" in CR["probeOtherRun"]):
    sys.exit(f"refusing: the receipt-against-ladder check must be live and its probe table must hold: {CR}")
if not all(E["skill"].values()):
    sys.exit(f"refusing: both skills, the process trigger and the verifier agent must name the receipt check: {E['skill']}")
S755 = E["sprint755"]
if not (S755["exists"] and S755["chartered"] and S755["gateResults"] == 6 and S755["storyDodPass"] and S755["dodOnBacklog"] and S755["retroNamesIssues"]):
    sys.exit(f"refusing: the check's delivery sprint must be chartered by the record with six gate results, both DoD results and a retro naming its issues: {S755}")
if not (E["issue603"]["declared"] and E["issue603"]["openIssuesExit0"] and E["issue603"]["open"] is False):
    sys.exit(f"refusing: the killed-receipt issue must be declared and resolved: {E['issue603']}")
for _n, _x in E["retroIssues"].items():
    if not (_x["declared"] and _x["resolver"]):
        sys.exit(f"refusing: retro issue {_n} must be declared with a resolver: {_x}")
if E["queue"]["decisionRows"] != QUEUE:
    sys.exit(f"refusing: the authority queue's decision rows must be the twelve: {E['queue']}")
N_PROBE_ROWS_R = CR["probeRows"]
N_FIXTURES_R = CR["fixtures"]
# the twelfth: the catalogue must carry the column on every row, every family must be a module file, no row may sit
# outside a table (issue667), the clause and its three tests must be in source, the guard must be green live, the sprint
# that shipped it must carry its results, the two issues it resolves must have left the open list and the tier issue stay open
CA, CO = H["catalogue"], H["code"]
if not (CA["rows"] == CA["rowsWithFamily"] == CA["enforcedGuards"] and not CA["rowsWithoutFamily"] and CA["familyHeaders"] == 2
        and CA["everyFamilyIsAModule"] and len(CA["families"]) == 9 and not CA["rowsOutsideATable"] and CA["preambleNamesTheColumn"]):
    sys.exit(f"refusing: every catalogue row must carry a family that is a module file and sit inside a table: {CA}")
if not all(CO.values()):
    sys.exit(f"refusing: the clause, its header constant and its four tests must be in source: {CO}")
if not (H["guardRun"]["exit0"] and "[guard:doc-guard-count] PASS" in H["guardRun"]["lastLine"]):
    sys.exit(f"refusing: doc-guard-count must be green live with the column in place: {H['guardRun']}")
S756 = H["sprint756"]
if not (S756["exists"] and S756["chartered"] and S756["gateResults"] == 6 and S756["storyDodPass"] and S756["dodOnBacklog"] and S756["retroNamesIssues"]):
    sys.exit(f"refusing: the clause's delivery sprint must be chartered by the record with six gate results, both DoD results and a retro naming its issues: {S756}")
for _n, _x in H["resolvedIssues"].items():
    if not (_x["declared"] and _x["open"] is False):
        sys.exit(f"refusing: issue{_n} must be declared and resolved by the DoD: {_x}")
if not (H["issue668"]["declared"] and H["issue668"]["resolver"] == "dcGuardTierHasOneHome" and H["issue668"]["open"] is True):
    sys.exit(f"refusing: the tier issue must be declared, open, with its own resolver: {H['issue668']}")
for _g in ("processChange", "docGuardCount"):
    if not (H["live"][_g]["exit0"] and H["live"][_g]["verdict"] == "PASS" and H["live"][_g]["violations"] == 0):
        sys.exit(f"refusing: the page says {_g} is green on this tree: {H['live'][_g]}")
if H["queue"]["decisionRows"] != QUEUE:
    sys.exit(f"refusing: the authority queue's decision rows must be the twelve: {H['queue']}")
N_ROWS, N_FAMILIES, N_HEADERS = CA["rows"], len(CA["families"]), CA["familyHeaders"]
N_SCANNED_DGC = H["live"]["docGuardCount"]["scanned"]
...
if not (Q["exit0"] and Q["byKind"].get("decisionAcceptance") == pending and Q["decisionRows"] == QUEUE):
    sys.exit(f"refusing: the authority queue's decision rows must be the twelve: {Q}")
...
N_PROPOSED_GATES = S752["gateProposed"] + S754["gateProposed"] + S755["gateProposed"] + S756["gateProposed"]
...
figR1 = logic_lanes(
    "Today the report is read alone; as the clause reads, against the tree",
    ("today", [
        ("retry lands", "filed refused", "accent", ""),
        ("report read alone", "count met", "bad", ""),
        ("eight gained", "seven claimed", "bad", ""),
    ], ["then", "so"]),
    ("as the clause reads", [
        ("tree gained", "eight", "accent", ""),
        ("fewer written", "refused", "ok", ""),
        ("unclaimed named", "", "ok", ""),
    ], ["then", "so"]),
)
figR3 = downstream(
    "One git read lands on one refusal, one brief rule and no guard",
    ("the report is read against the tree", "", ""),
    [
        ("checker", "refusal nine", "1", "ok"),
        ("probe table", "rows hold", str(N_PROBE_ROWS), "ok"),
        ("recorder brief", "rule seven", "1", "ok"),
        ("guards, hooks, CI", "unchanged", "0", "muted"),
    ],
)

figV1 = logic_lanes(
    "Today the receipt is read as fact; as the clause reads, against the ladder",
    ("today", [
        ("receipt says killed", "ladder running", "accent", ""),
        ("read as fact", "", "bad", ""),
        ("ladder ends red", "unrecorded", "bad", ""),
    ], ["then", "so"]),
    ("as the clause reads", [
        ("--wait", "till it ends", "accent", ""),
        ("receipt vs files", "at, rungs, counts", "ok", ""),
        ("disagreement", "refused", "ok", ""),
    ], ["then", "so"]),
)
figV3 = downstream(
    "One --wait and two file reads land on one refusal and no guard",
    ("the receipt is read against the run", "the ladder that ended", ""),
    [
        ("checker", "seven refusals", "1", "ok"),
        ("probe table", "rows hold", str(N_PROBE_ROWS_R), "ok"),
        ("recorder", "on a pass", "1", "ok"),
        ("guards, hooks, CI", "unchanged", "0", "muted"),
    ],
)

figC1 = logic_lanes(
    "A guard's code home moves from a grep to a gated cell",
    ("today", [
        ("row found", "family unknown", "accent", ""),
        ("grep the source", "", "bad", ""),
        ("guard moves", "row silent", "bad", ""),
    ], ["then", "so"]),
    ("as the clause reads", [
        ("row names family", "", "accent", ""),
        ("cell vs code", "held by the gate", "ok", ""),
        ("guard moves", "red, naming both", "ok", ""),
    ], ["then", "so"]),
)
figC3 = downstream(
    "One column and one clause land on one hard guard, no hook",
    ("every row names its family", "the gate holds the cell", ""),
    [
        ("catalogue rows", "carry a family", str(N_ROWS), "ok"),
        ("families", "each a module file", str(N_FAMILIES), "ok"),
        ("tables", "carry the column", str(N_HEADERS), "ok"),
        ("hooks, CI, workflows", "unchanged", "0", "muted"),
    ],
)

ASKS = [
    ("d0539", "ask-family", "Family", "family"),
    ("d0538", "ask-ladder", "Ladder", "ladder"),
    ("d0537", "ask-report", "Report", "report"),
...
panel_bodies = {
    "family": f"""<h2>Every guard row names the family that dispatches it</h2>
{clause(DEC["d0539"])}
<p><strong>What happened:</strong> nine family modules exist; the catalogue grouped rows by tier only, so a guard's code home was a grep. Twenty rows also sat outside any table, unnoticed.</p>
<p><strong>What changes:</strong> both tables gain a Family column naming the module that dispatches each guard, and the hard guard already reading the docs holds the cell to the code: an empty, foreign or wrong cell is a red gate naming the row and both families. A file without the column claims nothing.</p>
{figC1}
{figC3}
<p><strong>True:</strong> all {N_ROWS} rows carry a family inside a table; each of {N_FAMILIES} families is a module file; the guard is green over {N_SCANNED_DGC} files; both issues are closed. <strong>Mine:</strong> that the hard guard, not the warning-only one, is the right home. <strong>Decides it:</strong> whether a guard moved without its row fails the commit or only warns. <strong>Wrong if</strong> a downstream project hand-edits guards.md and wants no red gate there.</p>
{opts("ask-family", "d0539", [
    ("Accept: the row names the family, the gate holds it (recommended)", "ACCEPT: every guards.md row carries a Family cell that doc-guard-count holds equal to the dispatching FAMILY table, in any file whose header declares the column; a guard moved without its row is a red gate naming the row and both families (recommended)"),
    ("Column without the clause", "ALTERNATIVE: a unit test holds the column; doc-guard-count gains no clause"),
    ("Do nothing", "DO NOTHING: a guard's code home stays a grep"),
])}""",
    "ladder": f"""<h2>The verifier's receipt is read against the ladder that ended</h2>
{clause(DEC["d0538"])}
<p><strong>What happened:</strong> a verifier's receipt said the ladder was killed in its test rung while both lived. The ladder ended red fifteen minutes later: 880 passed, 23 failed. Nothing compared the two.</p>
<p><strong>What changes:</strong> a script waits until the ladder ends, then holds the receipt's ladder line, rung verdicts and test counts to the two files the ladder wrote, refusing one that says it did not end, quotes another run, or disagrees. A recorder is dispatched only on a pass.</p>
{figV1}
{figV3}
<p><strong>True:</strong> the probe table holds on all {N_PROBE_ROWS_R} rows over {N_FIXTURES_R} fixtures; the shipping sprint's receipt was checked twice - red ladder, then green; the killed-receipt issue is closed. <strong>Mine:</strong> that a timestamp, not the commit, pins a receipt to a run - one tree ran two ladders. <strong>Decides it:</strong> refuse or wait when no ended ladder backs the receipt. <strong>Wrong if</strong> a ladder can end without writing its receipt.</p>
{opts("ask-ladder", "d0538", [
    ("Accept: the receipt is read against the ladder (recommended)", "ACCEPT: check_receipt.py runs keel verify --wait, then refuses a receipt that says the ladder did not end, carries another run's at=, or disagrees with the two toml receipts on outcome, stopped_at, rung verdicts or touched counts; a recorder follows only a pass (recommended)"),
    ("Check the toml alone", "ALTERNATIVE: read the two receipt files without --wait; refuse only on disagreement"),
    ("Do nothing", "DO NOTHING: the rendered receipt is read as fact, uncompared with the ladder"),
])}""",
    "report": f"""<h2>The recorder's report is read against the tree it describes</h2>
{clause(DEC["d0537"])}
<p><strong>What happened:</strong> a recorder owed eight writes; one refused attempt landed on retry and was filed as refused. The checker reads the report alone, so seven written plus one refused met the count. The tree held eight; the report said seven.</p>
<p><strong>What changes:</strong> the checker counts the results the tree gained since the last commit and refuses a report with fewer written lines, naming each unclaimed result. One brief rule: a write that lands after a refusal is a written line. No guard, hook or workflow moves.</p>
{figR1}
{figR3}
<p><strong>True:</strong> the probe table holds on all {N_PROBE_ROWS} rows over this tree; the shipping sprint's report was the first judged this way, and passed; the issue is closed; the process-change guard is green with this record as charter. <strong>Mine:</strong> the one check wording cannot satisfy. <strong>Decides it:</strong> whether a result anyone else wrote onto the same uncommitted tree also refuses the report - the clause says yes; I commit mine first. <strong>Wrong if</strong> a ceremony must gain results nobody claims.</p>
{opts("ask-report", "d0537", [
    ("Accept: the report is read against the tree (recommended)", "ACCEPT: check_report.py under --root counts the TestResult parts the working tree gained since HEAD under .tracking and refuses a report with fewer WROTE lines than that, naming each gained result no WROTE key claims; a write that lands after a refused attempt is a WROTE line (recommended)"),
    ("Owed keys only", "ALTERNATIVE: the check refuses only when a result an owed --gate or --task key names is gained and unclaimed; a gained result outside the owed list is ignored"),
    ("Do nothing", "DO NOTHING: the checker reads the report alone and a landed write filed as REFUSED passes under the owed count"),
])}""",
...
<div class="ask"><p class="verdict"><strong>Take the recommended course on all twelve</strong>, accept each Need set; choose on every tab, then <em>Record</em>.</p></div>
...
<footer data-digest="provenance">Repository at {TREE}, {DATE}; {dirty} uncommitted files; {tests} tests, {failing} failing; {N_NEED_ROWS} of {N_NEEDS} Needs await your word; {N_PROPOSED_GATES} gates await your judgment; {N_OPEN} open issues. <a href="https://github.com/williamweatherholtz/sysmlv2-ai-toolkit" target="_blank" rel="noopener">Source</a>.</footer>
</div>
"""


TITLE = "Accept all twelve as recommended and the seven sets of Needs as written"
SUB = "Six forks, six process changes, and the Needs nobody asked about"
