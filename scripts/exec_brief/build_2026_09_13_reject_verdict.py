...
"""Build the standing decision brief for the 2026-09-13 (twenty-third) queue change: the twenty-second publish's six asks
stand (the retro name D0461, the keystone's second source D0465, the sample fork D0466, the Issue-first retro D0467, the
host-cost thresholds D0468, the measured-cost token D0469) and one joined them. D0470 is a safety-change ratification held
under D0337: `keel reject` records a human's rejection of a proposed Decision, and until sprint 702 nothing at the write
refused an AI-kind judgedBy, the computed control structure did not list reject as the human's action, and no rule read a
rejected Decision's rejection back - accept had all three (issue526). The three layers are applied on this tree; the
Decision ratifies them. The builder refuses unless the source reads the refusal inside reject's lock, the two command lists
agree, the rejection rule evaluates with zero violators and every rejected Decision in the tree is human-judged.
One tab per ask (D0404); the six carried tabs are the twenty-second builder's, every count a facts.py fact (sections 21-24),
never typed.

Usage: python scripts/exec_brief/build_2026_09_13_reject_verdict.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: six ratifications and one fork -------------------------------------------------------
QUEUE = ["d0461", "d0465", "d0466", "d0467", "d0468", "d0469", "d0470"]
...
if [p["slug"] for p in members] != QUEUE:
    sys.exit(f"refusing: this page is written for the retro name, the keystone, the sample fork, the Issue-first retro, the host-cost thresholds, the measured-cost token and the reject verdict; the queue is {[p['slug'] for p in members]}")
if AQ["decisions"] != QUEUE:
    sys.exit(f"refusing: the authority-queue lens reads {AQ['decisions']}; the file-derived set is {QUEUE}")
if [p["fork"] for p in members] != [False, False, True, False, False, False, False]:
...
<div class="ask"><p class="verdict"><strong>Accept the six controls</strong> and <strong>read the recall rule at {N100} cases</strong> so {MID} stands. <strong>Wrong if</strong> a one-position median move at {N50} cases is signal to you.</p></div>
...
TITLE = f"Accept six controls, and read the recall rule at {N100} cases so its setting stands"
SUB = "Six ratifications and one fork"
...
print(f"wrote {out_path}: {len(page)} bytes; frame {frame_words} words; panels " +
      ...
      f"; summed {words(page)}; rejected {RC['rejected']} all-human={RC['allHumanJudged']}; would-warn {PW['wouldWarn']} of {UNACC} unaccepted marked ({PROPOSED_MARKED} proposed, {REJECTED} rejected); "
