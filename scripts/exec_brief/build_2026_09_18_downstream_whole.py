...
# Read it once. Either it is a landing's green run, or a later commit's EMPTY run (that commit changed nothing under test;
# receiptHeadAfterLanding says it descends from the landing) and the landing's own run survives only as its log, written
# between the landing and the commit after it; each tab says which it shows.
_SMV0 = v("serveSitsAboveEveryMemberAndBelowTheBinary")["live"]
LATEST, LLOG = _SMV0["landingReceipt"], _SMV0["landingLog"]
LATEST_IS_LATER_EMPTY = bool(LATEST["exists"] and LATEST["outcome"] == "empty" and LATEST["stems"] == [] and LLOG and LLOG["receiptHeadAfterLanding"] is True)
...
CTR, CTLOG = CTL["touchedReceipt"], CTL["landingLog"]
LANDING_CT_IS_OWN = CTR["exists"] and CTR["head"] == CT["landed"]["range"][1] and CTR["outcome"] == "pass" and CTR["failed"] == "0"
if LANDING_CT_IS_OWN:
    N_LANDED_TESTS_CT, N_LANDED_FAILED_CT = int(CTR["passed"]), int(CTR["failed"])
    LANDED_ROW_CT = f"{N_LANDED_TESTS_CT} tests, {N_LANDED_FAILED_CT} failing"
elif (CTR["exists"] and CTLOG and CTLOG["receiptHeadAfterLanding"] is True and (CTR["outcome"] == "empty" or (CTR["outcome"] == "pass" and CTR["failed"] == "0"))
      and CTLOG["logsInWindow"] >= 1 and CTLOG["failed"] == 0 and CTLOG["run"] == CTLOG["passed"] and CTLOG["skipped"] == 0):
    # a later commit's run overwrote 743's receipt (one deep, D0421); 743's run is read from its log in its landing's window
    N_LANDED_TESTS_CT, N_LANDED_FAILED_CT = CTLOG["passed"], CTLOG["failed"]
    LANDED_ROW_CT = f"{N_LANDED_TESTS_CT} green from its log, {N_LANDED_FAILED_CT} failing"
else:
    sys.exit(f"refusing: the receipt is neither sprint 743's green landing nor a later commit's run beside 743's green log: {CTR} {CTLOG}")
CTLD = CT["landed"]
...
figCT3 = downstream(
    ...
    [
        ...
        ("the landing run", LANDED_ROW_CT, "1", "ok"),
