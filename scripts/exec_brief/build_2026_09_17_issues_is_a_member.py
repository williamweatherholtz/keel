...
"""Build the standing decision brief for the 2026-09-17 (forty-third) queue change: the twelve held records (ten process
changes - stems row, CI-triple lint, probe runner, wait command, guard tests, pair file, guard names, guard source, locked
anchor, suite member - and the two safety changes from issue578) are joined by a thirteenth held process change from sprint
737: the item and issue processes (github_ingest, intake_write, the issue write, the open-issues / dispositions / intake views)
left keel-cli, keel-write and keel-view for member keel-issues, and to depend on no guard and no view the resolver predicate
(declared_task_names, resolver_kind_holds) descended out of the guard member into keel_model::resolvers with re-exports at its
old paths, so the lock over members/keel-guards/src/ fired for two descents that change no guard logic; D0508 is the marked
record the lock asks for, and it reverses one clause of D0480 (a synopsis naming the member) with #SupersedeClause. Thirteen
tabs, thirteen asks (D0404); every count is a facts.py fact, never typed; each metric says in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_17_issues_is_a_member.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502", "d0503", "d0504", "d0505", "d0506", "d0508"]
...
_SMV0 = v("issuesReadsTheModelAndTheWriteApi")["live"]
...
LAST_GREEN_SPRINT = "sprint 737's"   # the latest green run; a tab whose own landing receipt is gone says whose it shows
...
if LANDING8_IS_OWN:
    ...
    sys.exit(f"refusing: the receipt is neither sprint 737's landing nor the next commit's empty run beside 737's log: {LR8}")
SP8 = IM["sprint737"]
...
_PF_LATER = {n: (f"pos{n}" in PFR["sides"][0] and f"neg{n}" in PFR["sides"][1]) for n in (736, 737)}
if not (_PF_733 or _PF_734 or _PF_735 or any(_PF_LATER.values())):
    sys.exit(f"refusing: the page says the latest ladder's pair is sprint 733's to 737's, positive then negative: {PFR['sides']}")
PAIR_ORDINAL = "eighth" if _PF_LATER[737] else "seventh" if _PF_LATER[736] else "sixth" if _PF_735 else "fifth" if _PF_734 else "fourth"
# the ordinal as a count of ladders in a row (the checker reads "eighth" as "eight h", a duration)
N_PAIR_LADDERS = {"fourth": 4, "fifth": 5, "sixth": 6, "seventh": 7, "eighth": 8}[PAIR_ORDINAL]
...
ASKS = [
    ("d0508", "ask-issuesmember", "Issues member", "issuesmember"),
...
<div class="ask"><p class="verdict"><strong>Accept all thirteen.</strong></p></div>
...
TITLE = "Accept the item member's descent and twelve more"
SUB = "Thirteen held, no fork"
