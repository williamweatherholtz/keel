...
(issue552: it WAS ready while chartered by this held Decision; a #DependsOn edge authored today holds it). D0484
...
Usage: python scripts/exec_brief/build_2026_09_14_facts_in_force_layering_build_skill.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: twelve ratifications and one fork -------------------------------------------------------
QUEUE = ["d0461", "d0465", "d0466", "d0467", "d0468", "d0469", "d0470", "d0471", "d0472", "d0473", "d0477", "d0483", "d0484"]
...
IT = WG["item"]
if not (IT["exists"] and IT["charteredByD0483"] and IT["dependsOnD0483"] and IT["dependsOnLeafSprint"] and IT["dodSaysBlocked"] and not IT["readyToday"]):
    sys.exit(f"refusing: the page says the delivering item is chartered by and now depends on d0483, says it is blocked, and is not ready; the record reads {IT}")
I552 = CBI["issue552"]
if not (I552["exists"] and I552["severity"] == "High" and I552["resolverAsExpected"] and I552["resolverDodNamesIssue"] and I552["namesBlockedBy"] and I552["resolverReadyRank"] == 1):
    sys.exit(f"refusing: the page says a High Issue naming blocked_by, resolved by dcReadyHonoursItemDependencies whose DoD names it and which ranks first; the record reads {I552}")
if CBI["blockedByReadsKinds"] != ["dependency", "dependson"] or CBI["blockedByReadsCharter"]:
    sys.exit(f"refusing: the page says the frontier reads two edge kinds and not the charter; the source reads {CBI['blockedByReadsKinds']}, charter={CBI['blockedByReadsCharter']}")
...
fig26 = downstream(
    ...
    [
        ...
        ("the finding", f"READY while chartered by this held ask; resolver ranks {n(I552['resolverReadyRank'])}", I552["severity"], "warn"),
    ],
)
...
<p><strong>True in the model:</strong> {n(len(WG['members']))} members, {n(len(WG['leafMembers']))} leaves, {n(len(WG['edgesAmongLeaves']))} edge among them; the delivering item is blocked by an edge I authored today: the frontier reads <code>{escape(", ".join(CBI['blockedByReadsKinds']))}</code>, not the charter, and ranked it ready while its DoD said blocked - a {I552['severity']} finding, resolver ranked {n(I552['resolverReadyRank'])}. <strong>Mine:</strong> that a convenience edge will be added upward is a forecast; the frontier's blindness is the finding. <strong>What decides it:</strong> whether a leaf reaching up to view or serve is a defect at the edit, or a choice each extraction makes. <strong>Wrong if</strong> you want the order moving with each extraction, not declared.</p>
...
print(f"wrote {out_path}: {len(page)} bytes; frame {frame_words} words; panels " +
      ...
      f"cli facts {CFF['inForce']}/{CFF['cliCommandLines']} in force, {CSL['line']}; layering: contract={WG['layersContractExists']} guard={WG['guardInNames']} item ready={IT['readyToday']} issue552 rank {I552['resolverReadyRank']}; "
