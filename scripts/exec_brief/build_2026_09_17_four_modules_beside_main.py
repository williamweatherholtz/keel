...
"""Build the standing decision brief for the 2026-09-17 (forty-fifth) queue change: the fifteen held records of the forty-fourth
page are joined by a sixteenth held process change, recorded BEFORE its sprint so acceptance holds the work: the four modules
still beside main.rs (adherence.rs and history.rs, the two audits that re-derive the gate verdict from the git tree; cursor.rs,
the process cursor; enroll.rs, actor enrollment) move to the members that own them - the audits into keel-guards, where the
directory lock covers them by prefix, the cursor and enrollment into keel-process. adherence.rs is named by GUARD_SOURCE_FILES,
so the move edits the locked constant: the file entry is retired (its file stays locked by the directory), history.rs enters
the lock it was never under, and D0209 clause 2 asks for a marked record held for the human before that commit lands. D0513 is
that record, standing on D0479 and D0209 and reversing nothing; nothing is applied. Sixteen tabs, sixteen asks (D0404); every
count is a facts.py fact, never typed; each metric says in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_17_four_modules_beside_main.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502", "d0503", "d0504", "d0505", "d0506", "d0508", "d0509", "d0510", "d0513"]
...
_PF_LATER = {n: (f"pos{n}" in PFR["sides"][0] and f"neg{n}" in PFR["sides"][1]) for n in (736, 737, 738, 739, 740)}
if not (_PF_733 or _PF_734 or _PF_735 or any(_PF_LATER.values())):
    sys.exit(f"refusing: the page says the latest ladder's pair is sprint 733's to 740's, positive then negative: {PFR['sides']}")
# the pair's sprint, as a count of ladders in a row since 733's (the fourth): 733 -> 4 ... 740 -> 11
_PF_SPRINT = next((n for n in (740, 739, 738, 737, 736) if _PF_LATER[n]), 735 if _PF_735 else 734 if _PF_734 else 733)
...
<div class="ask"><p class="verdict"><strong>Accept all sixteen.</strong></p></div>
...
TITLE = "Accept sixteen: modules go home"
SUB = "Sixteen held, no fork"
