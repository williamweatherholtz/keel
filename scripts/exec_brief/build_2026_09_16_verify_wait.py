...
"""Build the standing decision brief for the 2026-09-16 (thirty-fifth) queue change: the three held process changes
(stems row, CI-triple lint, probe runner) are joined by a fourth - the wait for a launched ladder is a command,
keel verify --wait, that blocks while the receipt's writer pid is alive and reports a dead writer as KILLED, never a
verdict, so the sentence a verifier skipped (issue575) is no longer the control. Four tabs, four asks (D0404); every
count is a facts.py fact, never typed; each metric says in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_16_verify_wait.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: four held process changes ------------------------------------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0494", "d0495", "d0496", "d0497"]
...
print(f"wrote {out_path}: {len(page)} bytes; frame {frame_words} words; panels " +
      ...
      f"728 {N_RESULTS8}+{N_GATE_RESULTS8}; wait: {N_PROBE_TESTS} tests, {N_WAIT_STEPS} steps, epoch item {POS_EPOCH}, git-read item {POS_GITREAD}")
