...
"""Build the standing decision brief for the 2026-09-16 (thirty-seventh) queue change: the five held process changes
(stems row, CI-triple lint, probe runner, wait command, guard tests) are joined by a sixth - the D0388 probe pair
reaches the verifier's ladder as a two-line file the primary wrote (keel verify --probe-from FILE), and the verifier
names the path instead of retyping two command lines into a comma-separated argument. Six tabs, six asks (D0404);
every count is a facts.py fact, never typed; each metric says in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_16_probe_pair_file.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: six held process changes -------------------------------------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500"]
...
# -- ask 6 (first tab): the probe pair travels in a file --
...
PFR = PFL["probeRung"]
if not (PFR and PFR["verdict"] == "pass" and PFR["namesAFile"] and PFR["sidesAreTheTests"] and len(PFR["sides"]) == 2):
    sys.exit(f"refusing: the page says the probe rung ran from a named file whose two lines are the flag's own pair: {PFR}")
...
<div class="ask"><p class="verdict"><strong>Accept all six</strong>: pair file, guard-test fix, wait command, pre-commit probes and lint, stems row.</p></div>
<div class="chips"><span class="chip"><b>Decisions waiting</b> {pending}</span><span class="chip"><b>Forks</b> 0</span><span class="chip"><b>Shapes refused</b> {N_REFUSALS}</span><span class="chip"><b>Locked lines signed</b> {N_HUNKS}</span></div>
...
TITLE = "Accept the pair file, guard-test fix, wait command, probe runner, CI-triple lint and stems row"
SUB = "Six held process changes, no fork"
