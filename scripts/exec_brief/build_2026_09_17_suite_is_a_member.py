...
"""Build the standing decision brief for the 2026-09-17 (forty-second) queue change: the eleven held records (nine process
changes - stems row, CI-triple lint, probe runner, wait command, guard tests, pair file, guard names, guard source, locked
anchor - and the two safety changes from issue578) are joined by a twelfth held process change from sprint 735: the
build-and-test tooling (suite, touched, verify, hook_binary, contentkey) left keel-cli for member keel-suite, and to depend on
no view and no guard it took the content key out of the guard member and moved the no-receipt predicate down to keel-fs, so
the lock over members/keel-guards/src/ fired for two descents that change no guard logic (lib.rs 1 line out, receipt.rs 6 out
and 3 in) and D0506 is the marked record the lock asks for. Twelve tabs, twelve asks (D0404); every count is a facts.py fact,
never typed; each metric says in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_17_suite_is_a_member.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: ten held process changes and two held safety changes -------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502", "d0503", "d0504", "d0505", "d0506"]
...
_SMV0 = v("suiteDependsOnNoViewOrGuard")["live"]
...
LAST_GREEN_SPRINT = "sprint 735's" if LATEST_IS_LATER_EMPTY else None   # set per tab below when the receipt is its own
...
if not LANDING4_IS_OWN and not LATEST_IS_LATER_EMPTY:
...
LR5 = SMV["landingReceipt"]
LANDING5_IS_OWN = LR5["exists"] and LR5["head"] == SML["range"][1] and LR5["outcome"] == "pass"
if not (LANDING5_IS_OWN or LATEST_IS_LATER_EMPTY):
    sys.exit(f"refusing: the receipt is neither sprint 735's landing nor the next commit's empty run beside 735's log: {LR5}")
if LANDING5_IS_OWN:
    ...
    # the landing's receipt was overwritten by the next commit's empty run; the run itself is read from its log (landingLog)
    N_LANDED_TESTS5, N_LANDED_FAILED5, N_STEMS5 = LLOG["passed"], LLOG["failed"], LLOG["binaries"]
...
if not (LANDING3_IS_OWN or LANDING3_IS_734 or LATEST_IS_LATER_EMPTY):
...
if not (_PF_733 or _PF_734 or _PF_735):
    sys.exit(f"refusing: the page says the latest ladder's pair is sprint 733's, 734's or 735's, positive then negative: {PFR['sides']}")
PAIR_ORDINAL = "sixth" if _PF_735 else "fifth" if _PF_734 else "fourth"
...
ASKS = [
    ("d0506", "ask-suitemember", "Suite member", "suitemember"),
...
<div class="ask"><p class="verdict"><strong>Accept all twelve.</strong></p></div>
...
TITLE = "Accept the tooling member's descents and eleven more"
SUB = "Twelve held, no fork"
