...
"""Build the standing decision brief for the 2026-09-18 (forty-seventh) queue change: the seventeen held records of the forty-sixth
page are joined by an eighteenth held process change, applied nowhere: a sprint Story names the backlog item it delivers with a
#Delivers edge authored at prep beside the charter edge, and the sprint-closure guard holds each delivered item to a DoD
TestResult once a later sprint exists - a post-acceptance sprint with no edge is red too, pre-acceptance sprints are history and
nothing backfills by guessing. Twice (sprints 720 and 735) a sprint closed with its story DoD green and the item it existed to
deliver ranked first as READY, the only link between them a shared slug and a prose line in the fill. D0515 stands on the charter
lineage, the sprint-closure rule and the enforcement surface; held because schema/core is frozen and the guard change rides the
lock. Eighteen tabs, eighteen asks (D0404); every count is a facts.py fact, never typed; each metric says in plain words what it
is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_18_delivers.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502", "d0503", "d0504", "d0505", "d0506", "d0508", "d0509", "d0510", "d0513", "d0514", "d0515"]
...
# -- ask 18 (first tab): a sprint Story names the item it delivers with a #Delivers edge, and sprint-closure reads it - held, nothing applied --
...
if not (all(CTS["docLinesRepointed"].values()) and all(CTS["docLinesWereStale"].values()) and CTS["migrateLine804NamesTheFn"]):
    sys.exit(f"refusing: the page says both doc lines said 664-666 before and say 804-807 now, and line 804 names the fn: {CTS['docLinesBefore']} {CTS['docLinesToday']} {CTS['migrateLine804']}")
N_CT_TESTS, N_CT_HELPERS, L_MIGRATE = CTS["testCount"], len(CTS["helpers"]), CTS["migrateLines"]
CTL = CT["live"]
...
if not (_PF_733 or _PF_734 or _PF_735 or any(_PF_LATER.values())):
    sys.exit(f"refusing: the page says the latest ladder's pair is sprint 733's to 743's, positive then negative: {PFR['sides']}")
# the pair's sprint, as a count of ladders in a row since 733's (the fourth): 733 -> 4 ... 743 -> 14
_PF_SPRINT = next((n for n in (743, 742, 741, 740, 739, 738, 737, 736) if _PF_LATER[n]), 735 if _PF_735 else 734 if _PF_734 else 733)
...
figCT1 = logic_lanes(
    ...
    ("before", [
        ("module moves", "fn at 804 today", "accent", ""),
...
figCT3 = downstream(
    ...
    [
        ...
        ("the two doc lines", "repointed to 804-807", "2", "ok"),
...
{clause(CT)}
<p><strong>What happened:</strong> a module moved; two living docs kept citing lines 664-666 of it. The function they name sits at line 804; nobody read the citation. The guard's first run reddened every scaffolded project: their docs cite source they lack.</p>
...
<div class="ask"><p class="verdict"><strong>Accept all eighteen.</strong></p></div>
...
TITLE = "Accept eighteen: name deliveries"
SUB = "Eighteen held, unforked"
