...
"""Build the standing decision brief for the 2026-09-16 (thirty-sixth) queue change: the four held process changes
(stems row, CI-triple lint, probe runner, wait command) are joined by a fifth - the seven fixed-name scratch joins in
guards.rs's test module are rewritten to keel_fs::scratch under the D0209 clause 2 lock, and the record the lock asks
for is a held Decision rather than a carve-out for test regions. Five tabs, five asks (D0404); every count is a
facts.py fact, never typed; each metric says in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_16_guard_tests_scratch.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: five held process changes ------------------------------------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499"]
...
<div class="ask"><p class="verdict"><strong>Accept all five</strong>: sign the guard-test fix; the wait is a command; CI's probes and lint run pre-commit; the stems row matches the binary.</p></div>
<div class="chips"><span class="chip"><b>Decisions waiting</b> {pending}</span><span class="chip"><b>Forks</b> 0</span><span class="chip"><b>Locked lines signed</b> {N_HUNKS}</span><span class="chip"><b>CI-only reds, the class</b> {N_CLASS}</span></div>
...
TITLE = "Accept the guard-test fix, wait command, probe runner, CI-triple lint and stems row"
SUB = "Five held process changes, no fork"
