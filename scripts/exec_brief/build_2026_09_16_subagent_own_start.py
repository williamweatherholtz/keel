...
"""Build the standing decision brief for the 2026-09-16 (thirty-eighth) queue change: the six held process changes
(stems row, CI-triple lint, probe runner, wait command, guard tests, pair file) are joined by two held SAFETY changes
from issue578 - a subagent is measured from the tree at its OWN start (D0501: the dispatcher writes agent-{id}.fp,
SubagentStart is the seventh hook event) and a verifier's stop is never a block (D0502: a verifier that wrote is one
refused-class ledger line under verifier:tree-written, beside recorder:tree-red). Eight tabs, eight asks (D0404);
every count is a facts.py fact, never typed; each metric says in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_16_subagent_own_start.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: six held process changes and two held safety changes ---------------------------------
pending = v("pendingAcceptances")
members = v("pendingMembers")
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502"]
...
# -- asks 7 and 8 (first two tabs): a verifier's stop is never a block; a subagent is measured from its own start --
...
PFR = PFL["probeRung"]
# the receipt on disk is the LATEST ladder's - sprint 731's, whose pair names the stop-route tests (the second sprint to carry
# its pair in a file); the page says which file and whose tests, it does not require sprint 730's own pair to still be there
...
if not all(t in " ".join(PFR["sides"]) for t in SVS["tests"][:2]):
    sys.exit(f"refusing: the page says the latest ladder's pair is the stop route's two tests: {PFR['sides']} vs {SVS['tests'][:2]}")
...
<p><strong>True:</strong> {N_REFUSALS} wrong shapes refused live, exit 2, receipt untouched; the latest ladder - the second sprint to use it - ran its pair from <q>{PAIR_FILE}</q>, {N_RUNGS_PF} rungs green. <strong>Mine:</strong> a path is the one carrier a verifier cannot mis-copy. <strong>What decides it:</strong> may a subagent retype an argument another actor chose. <strong>Wrong if</strong> a verifier ever needs a pair the primary did not write.</p>
...
<div class="ask"><p class="verdict"><strong>Accept all eight</strong>; the first two are safety changes.</p></div>
<div class="chips"><span class="chip"><b>Waiting</b> {pending}</span><span class="chip"><b>Safety changes</b> 2</span><span class="chip"><b>Forks</b> 0</span><span class="chip"><b>Blocks that day</b> {N_BLOCKS_DAY}</span></div>
...
TITLE = "Accept the never-blocked verifier, the own-start baseline and six process changes"
SUB = "Eight held, two of them safety changes, no fork"
