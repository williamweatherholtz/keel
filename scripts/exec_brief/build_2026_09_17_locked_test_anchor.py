...
"""Build the standing decision brief for the 2026-09-17 (forty-first) queue change: the ten held records (eight process
changes - stems row, CI-triple lint, probe runner, wait command, guard tests, pair file, guard names, guard source - and the
two safety changes from issue578) are joined by an eleventh held process change from sprint 734: one repository-root walk
for every test (keel_fs::test_support::repo_root) replaced ten local helper copies and eight cwd-relative anchors, and one of
those anchors sat inside keel-cli/src/adherence.rs, a locked guard source, so the process-change lock fired for a two-line
test-only diff and D0505 is the marked record the lock asks for - the first marked Decision since D0209 that governs no
change to enforcement logic. Eleven tabs, eleven asks (D0404); every count is a facts.py fact, never typed; each metric says
in plain words what it is doing (issue562).

Usage: python scripts/exec_brief/build_2026_09_17_locked_test_anchor.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
QUEUE = ["d0494", "d0495", "d0496", "d0497", "d0499", "d0500", "d0501", "d0502", "d0503", "d0504", "d0505"]
...
if not (LR4["exists"] and LR4["head"] == LAL["range"][1] and LR4["outcome"] == "pass"):
    sys.exit(f"refusing: the landing receipt must be the landing commit's and green: {LR4}")
N_LANDED_TESTS4, N_LANDED_FAILED4 = int(LR4["passed"]), int(LR4["failed"])
I557 = LA["issue557"]
...
if not (LR3["exists"] and LR3["outcome"] == "pass" and LR3["stemsIncludeInit"]):
    sys.exit(f"refusing: the landing receipt must be green and carry the init stem: {LR3}")
# the receipt is machine-local and one deep: once sprint 734 landed it is 734's, and this tab says so rather than quoting it as 733's
LANDING3_IS_OWN = LR3["head"] == GPL["range"][1]
if not LANDING3_IS_OWN and LR3["head"] != LAL["range"][1]:
    sys.exit(f"refusing: the receipt is neither sprint 733's landing nor sprint 734's: {LR3['head']}")
N_LANDED_TESTS, N_LANDED_FAILED = int(LR3["passed"]), int(LR3["failed"])
LANDING3_ROW = f"{N_LANDED_TESTS} tests, {N_LANDED_FAILED} failing" if LANDING3_IS_OWN else f"sprint 734's: {N_LANDED_TESTS} tests, {N_LANDED_FAILED} failing"
LANDING3_TRUE = f"{N_LANDED_TESTS} tests green at the landing" if LANDING3_IS_OWN else f"now sprint 734's, {N_LANDED_TESTS} green"
...
if not (_PF_733 or _PF_734):
    sys.exit(f"refusing: the page says the latest ladder's pair is sprint 733's or 734's, positive then negative: {PFR['sides']}")
PAIR_ORDINAL = "fifth" if _PF_734 else "fourth"
...
figE3 = downstream(
    ...
    [
        ...
        ("the landing run", f"{N_LANDED_TESTS4} tests, {N_LANDED_FAILED4} failing", "1", "ok"),
...
<p><strong>True:</strong> {N_REFUSALS} wrong shapes refused live, exit 2, receipt untouched; the latest ladder - the {PAIR_ORDINAL} sprint to use it - ran its pair from <q>{PAIR_FILE}</q>, {N_RUNGS_PF} rungs green. <strong>Mine:</strong> a path is the one carrier a verifier cannot mis-copy. <strong>What decides it:</strong> may a subagent retype an argument another actor chose. <strong>Wrong if</strong> a verifier ever needs a pair the primary did not write.</p>
...
<div class="ask"><p class="verdict"><strong>Accept all eleven</strong>; two are safety changes.</p></div>
...
TITLE = "Accept the locked test anchor's repoint and ten more changes"
SUB = "Eleven held, two safety changes, no fork"
