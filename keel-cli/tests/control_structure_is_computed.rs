...
fn a_fresh_project_computes_its_structure_with_no_authored_anchors() {
    ...
    assert!(acts.iter().any(|n| n == "cmdRecordTask") && !acts.iter().any(|n| n == "cmdRecord") && fbs.iter().any(|n| n == "readOrient"), "CLI facts yield actions and feedback - one per sub-verb of a routing writer, never the bare router (D0454): {acts:?}");
