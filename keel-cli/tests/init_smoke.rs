...
fn init_scaffolds_a_working_project() {
    ...
    // path; CI caught its absence from this list the day it landed - and check_receipt.py, D0538 - the
    // verifier brief runs it by path; the sprint 755 ladder caught its absence). Exactly these, nothing else.
    ...
    assert_eq!(
        names,
        vec!["check_receipt.py", "check_report.py", "deck_inbox_record.py", "test_deck_e2e.py"],
